...
check_permissions() {
    ...
    if [ "$current_permission" -eq "$permission" ]; then
...
if [ "$sysctl_ip_forwarding" -eq 0 ]; then
    echo "IP forwarding is disabled."
else
    echo "IP forwarding is not disabled. Remediation: Set 'net.ipv4.ip_forward' to '0' in sysctl."
fi

# Check if ICMP redirects are not accepted
sysctl_icmp_redirects=$(sysctl net.ipv4.conf.all.accept_redirects | awk '{print $3}')
if [ "$sysctl_icmp_redirects" -eq 0 ]; then
    echo "ICMP redirects are not accepted."
else
    echo "ICMP redirects are accepted. Remediation: Set 'net.ipv4.conf.all.accept_redirects' to '0' in sysctl."
fi

# Check permissions for specified files
check_permissions "/etc/crontab" 600 "Set permissions to 600."
